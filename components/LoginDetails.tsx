...
const EmailDetailsForm = ({ email }: { email: string | null }) => {
  ...
  type ChangeEmailFormData = yup.InferType<typeof schemaChangeEmail>;
...
const PasswordDetailsForm = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (!result.errors?.length) {
      setGeneralMessage(t('logindetails.passworddetails.success'));
      reset();
    } else {
  ...
  return (
    <form className="mt-16 flex flex-col" onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-3">
        <PasswordField
          control={control}
      ...
      >
        {t('logindetails.submitbuttontext')}
      </Button>
      {!!generalMessage && (
        <Text
          className="mt-2 text-sm"
          data-testid="password-details__general-message"
        >
          {generalMessage}
        </Text>
      )}
