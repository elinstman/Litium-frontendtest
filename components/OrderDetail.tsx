...
import { DiscountType, OrderStatus, OrderTags } from 'utils/constants';
...
function OrderDetail(props: OrderDetailProps) {
  ...
  const totalDiscounts =
    (order?.discountInfos &&
      Math.abs(calculateTotalDiscounts(order?.discountInfos))) ||
    0;
  const productDiscountLines = order?.discountInfos?.filter(
    (item: DiscountInfo) =>
  ...
  const totalProductDiscounts =
    (productDiscountLines &&
      Math.abs(calculateTotalDiscounts(productDiscountLines))) ||
    0;
  const itemsSubtotal =
    (order?.productTotalIncludingVat || 0) - totalProductDiscounts;
  const totalOtherDiscounts = totalDiscounts - totalProductDiscounts;
  const shippingFeeLine = order?.rows?.filter(
    (item: OrderRow) => item.rowType === 'SHIPPING_FEE'
  )[0];
  const paymentFeeLine = order?.rows?.filter(
    (item: OrderRow) => item.rowType === 'FEE'
  )[0];
        ...
        <div className="mb-10 w-full lg:w-1/2">
          ...
          <DetailLine
            ...
            value={
              <Currency
                className="inline"
                price={paymentFeeLine?.totalIncludingVat || 0}
          ...
          <DetailLine
            ...
            value={
              <Currency
                className="inline"
                price={shippingFeeLine?.totalIncludingVat || 0}
